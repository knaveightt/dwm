...
enum { SchemeNorm, SchemeSel, SchemeHid, SchemeButn, SchemeLayt }; /* color schemes */
...
buttonpress(XEvent *e)
{
	...
	if (ev->window == selmon->barwin) {
		...
		if (i < LENGTH(tags)) {
			...
			click = ClkLtSymbol;
		/* adding 2px right padding for awesomebar patch, as well as systray width */
...
drawbar(Monitor *m)
{
  int x, w, wdelta, tw = 0, stw=0, n = 0, scm;
	...
	w = TEXTW(m->ltsymbol);
	drw_setscheme(drw, scheme[SchemeLayt]); /* layout menu icon color scheme */
	...
	if ((w = m->ww - tw - stw - x) > bh) {
		if (n > 0) {
			...
			for (c = m->clients; c; c = c->next) {
				...
				/* updates to include winicons next to client window names */
